...
export default function UpdateWorkExperience({ resumeId, updateResumeValues }) {
    ...
    useEffect(() => {
        let workExperienceService = new WorkExperienceService();
        workExperienceService.getAllByResumeIdOrderByEndYearDesc(resumeId).then((result) => {
            setWorkExperiences(result.data.data)
        })
    }, [resumeId])

    let workExperienceAddSchema = Yup.object().shape({
        workplaceName: Yup.string().required("Zorunlu alan").min(2, "En az 2 karakter uzunluğunda olmalıdır"),
        position: Yup.string().required("Zorunlu alan").min(2, "En az 2 karakter uzunluğunda olmalıdır"),
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            workExperienceService.add(values).then((result) => {
                    ...
                    <Table.Body>
                        {workExperiences?.map((workExperience) => (
                            <Table.Row key={workExperience.id}>
                                ...
                                <Table.Cell>
                                    <Button color="red" icon="x" circular onClick={() => handleDeleteWorkExperience(workExperience.id)}>
                                    </Button>
                                </Table.Cell>
                            </Table.Row>
                        ))}
